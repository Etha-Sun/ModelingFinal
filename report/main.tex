...
图 \ref{fig:heatmap} 说明，在官方二维数据和 2000 个训练样本设置下，KDE 的 MMD 排名最稳定。这并不代表 KDE 在所有生成任务中都优于神经模型，而是说明低维、样本充分、评价分布与训练分布同机制时，直接利用样本邻域的非参数估计效果很强。这个结果进一步论证了本文的核心观点：生成建模不能简单按模型新旧排序，而要分析数据维度、样本量、支撑几何和评价指标。
...
\begin{enumerate}
    ...
    \item AI辅助书写了问题提出实验方法等部分的文本内容，提供了数学公式和实验设计细节；
