...
实验使用题目提供的 \texttt{distribution2d\_gen/generate\_data.py} 生成四类训练集、测试集和隐藏测试集，每类训练样本 2000 个、测试样本 2000 个，并在 3 个随机种子上重复训练与采样。评价指标包括负对数似然（NLL）、最大均值差异（MMD）、切片 Wasserstein 距离（SWD）、Precision、Coverage 和二维网格支撑覆盖率。结果显示，在该二维生成器和当前样本量设置下，KDE 是最稳定的基线：它在测试集和隐藏测试集上都取得了最好的整体样本匹配结果；GMM 在 Gaussian Mixture、Ring 和 Two Moons 上具有较高 Precision 与较低 NLL，但在 Spiral 上受椭圆高斯局部假设限制；DDPM 在 Ring 和 Two Moons 上能生成大致形状，但在 Spiral 上仍明显偏散；VAE 训练稳定，但生成样本过于平滑。我们还完成了条件 DDPM、超参数扫描和四类模型的异常点鲁棒性实验。整体来看，在这个低维合成任务上，模型新旧不是最重要的，模型假设是否贴合数据形状更关键。
\end{abstract}
...
数据来自题目提供的二维分布生成代码 \texttt{distribution2d\_gen/generate\_data.py}。该脚本默认每类生成 2000 个训练样本、2000 个测试样本和 2000 个隐藏测试样本，并保存 \texttt{train.npy}、\texttt{test.npy}、标签文件和 \texttt{metadata.json}。主实验使用训练集拟合模型、测试集评价生成质量，同时保留一份独立隐藏测试集作为泛化检查；为估计随机性，实验脚本复现同一生成机制，并在不同随机种子下重新生成训练集、测试集和隐藏测试集。四类分布的构造如下：
\begin{itemize}
...
\begin{itemize}
    ...
    \item \texttt{metrics\_summary.csv/json}：测试集指标的均值和标准差；
    \item \texttt{hidden\_metrics\_summary.csv/json}：隐藏测试集指标的均值和标准差；
    \item \texttt{figures/*.pdf}：报告图表；
    \item \texttt{tables/*.tex}：可直接插入报告的表格；
    \item \texttt{conditional\_metrics\_summary.csv}：条件 DDPM 测试集指标；
    \item \texttt{conditional\_hidden\_metrics\_summary.csv}：条件 DDPM 隐藏测试集指标；
...
\centering
\scriptsize
\caption{四类模型在隐藏测试集上的主指标，均为 3 个随机种子的均值 \(\pm\) 标准差。NLL 只对 KDE 和 GMM 报告。}
\resizebox{\textwidth}{!}{\input{../results/tables/hidden_results_table.tex}}
\label{tab:hidden}
\end{table}

表 \ref{tab:hidden} 使用未参与训练、调参和主结果讨论的隐藏测试集做补充检查。hidden split 上的排序与测试集基本一致：KDE 在四类分布上仍取得最低 MMD 和 SWD，并保持最高 Coverage；GMM 在 Gaussian Mixture、Ring 和 Two Moons 上的 NLL 与 Precision 仍较好；VAE 的过平滑问题没有消失；DDPM 在 Ring 和 Two Moons 上接近真实形状，但 Spiral 上 Precision 仍偏低。因此，主结论不是由某一次测试集抽样偶然造成的。
...
图 \ref{fig:heatmap} 中，KDE 在题目给定的二维数据和 2000 个训练样本设置下排名最稳定。这并不代表 KDE 在所有生成任务中都优于神经模型。更准确地说，当数据维度低、样本量足够、测试分布和训练分布来自同一机制时，直接利用样本邻域的非参数估计效果很强。低维生成任务不能简单按模型新旧排序，还要看数据形状和评价指标。
...
\begin{table}[H]
\centering
\caption{条件 DDPM 在隐藏测试集上的生成质量，均为 3 个随机种子的均值 \(\pm\) 标准差。}
\input{../results/tables/conditional_hidden_results_table.tex}
\label{tab:conditional-hidden}
\end{table}

图 \ref{fig:conditional}、表 \ref{tab:conditional} 和表 \ref{tab:conditional-hidden} 可以看出，类别标签确实起到了控制作用，但质量并不均衡。Ring 较清楚，Two Moons 能看出月牙轮廓；Gaussian Mixture 被拉成近似环状，Spiral 仍偏散。与单独训练的 DDPM 相比，条件 DDPM 在 Gaussian Mixture、Ring 和 Spiral 上的 MMD 更高。隐藏测试集上，Gaussian Mixture 的 Precision 只有 \(0.510\pm0.055\)，Spiral 的 MMD 为 \(0.0028\pm0.0010\)，说明多分布共享参数确实带来了干扰。
...
\begin{enumerate}
    \item 在给定的二维生成器和每类 2000 个训练样本设置下，KDE 是最稳定的基线，在测试集和隐藏测试集上都取得最低 MMD、最低 SWD 和最高 Coverage；
...
对于改进方向，可以考虑加入 RealNVP 或 Neural Spline Flow 作为精确似然神经模型。这样可以在保持神经模型表达能力的同时，对 NLL 做更公平的横向比较，也能检验神经生成模型是否能在该二维数据上同时取得好样本距离和好似然。
...
\texttt{data/} & 数据生成脚本默认 seed 生成的 train/test/hidden 数据文件 \\
...
\texttt{results/hidden\_metrics\_summary.csv} & 隐藏测试集聚合指标 \\
\texttt{conditional\_metrics\_summary.csv} & 条件 DDPM 测试集聚合指标，位于 \texttt{results/} 目录 \\
\texttt{conditional\_hidden\_metrics\_summary.csv} & 条件 DDPM 隐藏测试集聚合指标，位于 \texttt{results/} 目录 \\
...
\begin{enumerate}
    ...
    \item 孙悦淳：生成模型建立与实验实现部分，包括实现 KDE、GMM、VAE 和 DDPM 四类生成模型，完成模型训练、样本生成、条件 DDPM 拓展实验，以及相关超参数设置和复现实验脚本整理，并完成实验报告初版撰写；
...
\begin{enumerate}
    \item 使用AI根据数学建模标准报告模板给出本文的标准\LaTeX{} 报告结构和章节标题；
    \item AI辅助书写了问题提出实验方法等部分的文本内容；
    \item AI帮助整理了实验结果的表格和图像说明；
    \item AI根据已有的代码进行了重新排版和变量命名，增强了代码的可读性。
\end{enumerate}
