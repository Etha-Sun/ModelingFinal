...
数据来自题目提供的二维分布生成代码 \texttt{distribution2d\_gen/generate\_data.py}。官方脚本默认每类生成 2000 个训练样本、2000 个测试样本和 2000 个隐藏测试样本，并保存 \texttt{train.npy}、\texttt{test.npy}、标签文件和 \texttt{metadata.json}。主实验使用训练集拟合模型、测试集评价生成质量，同时保留一份独立隐藏测试集作为泛化检查；为估计随机性，实验脚本复刻官方生成机制，并在不同随机种子下重新生成训练集、测试集和隐藏测试集。四类分布的构造如下：
\begin{itemize}
...
AI 工具用于辅助整理报告结构、检查文字表达和排版细节。实验数据、指标数值和图像均由本地脚本运行生成，没有手工编造。最终结论根据主指标表、隐藏测试集指标表、图像文件和运行清单核对后写入报告。
